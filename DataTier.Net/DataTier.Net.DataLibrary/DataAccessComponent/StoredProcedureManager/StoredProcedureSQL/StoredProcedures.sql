...
BEGIN
    ...
    ([ClassFileName],[ClassName],[DatabaseId],[Exclude],[Excluded],[IsView],[ProjectId],[Scope],[Serializable],[TableName])

    -- Begin Values List
    Values(@ClassFileName, @ClassName, @DatabaseId, @Exclude, @Excluded, @IsView, @ProjectId, @Scope, @Serializable, @TableName)
...
BEGIN
    ...
    -- Begin Select Statement
    Select [DataType],[Exclude],[FieldName],[IsNullable],[ProjectId],[TableId],[TableName]
...
BEGIN
    ...
    -- Begin Insert Statement
    Insert Into [Project]
    ([AddIGridValueInterface],[AutoFillPaths],[ControllerFolder],[ControllerNamespace],[ControllerReferencesSetId],[DataManagerFolder],[DataManagerNamespace],[DataManagerReferencesSetId],[DataOperationsFolder],[DataOperationsNamespace],[DataOperationsReferencesSetId],[DataWriterFolder],[DataWriterNamespace],[DataWriterReferencesSetId],[DateModified],[GatewayNamespace],[GatewayPath],[ObjectFolder],[ObjectNamespace],[ObjectReferencesSetId],[ProjectFolder],[ProjectName],[ReaderFolder],[ReaderNamespace],[ReaderReferencesSetId],[StoredProcedureObjectFolder],[StoredProcedureObjectNamespace],[StoredProcedureReferencesSetId],[StoredProcsFolder],[TargetFramework],[TemplateVersion])

    -- Begin Values List
    Values(@AddIGridValueInterface, @AutoFillPaths, @ControllerFolder, @ControllerNamespace, @ControllerReferencesSetId, @DataManagerFolder, @DataManagerNamespace, @DataManagerReferencesSetId, @DataOperationsFolder, @DataOperationsNamespace, @DataOperationsReferencesSetId, @DataWriterFolder, @DataWriterNamespace, @DataWriterReferencesSetId, @DateModified, @GatewayNamespace, @GatewayPath, @ObjectFolder, @ObjectNamespace, @ObjectReferencesSetId, @ProjectFolder, @ProjectName, @ReaderFolder, @ReaderNamespace, @ReaderReferencesSetId, @StoredProcedureObjectFolder, @StoredProcedureObjectNamespace, @StoredProcedureReferencesSetId, @StoredProcsFolder, @TargetFramework, @TemplateVersion)
...
BEGIN
    ...
    -- Update Each field
    Set [AddIGridValueInterface] = @AddIGridValueInterface,
    [AutoFillPaths] = @AutoFillPaths,
...
BEGIN
    ...
    -- Begin Select Statement
    Select [AddIGridValueInterface],[AutoFillPaths],[ControllerFolder],[ControllerNamespace],[ControllerReferencesSetId],[DataManagerFolder],[DataManagerNamespace],[DataManagerReferencesSetId],[DataOperationsFolder],[DataOperationsNamespace],[DataOperationsReferencesSetId],[DataWriterFolder],[DataWriterNamespace],[DataWriterReferencesSetId],[DateModified],[GatewayNamespace],[GatewayPath],[ObjectFolder],[ObjectNamespace],[ObjectReferencesSetId],[ProjectFolder],[ProjectId],[ProjectName],[ReaderFolder],[ReaderNamespace],[ReaderReferencesSetId],[StoredProcedureObjectFolder],[StoredProcedureObjectNamespace],[StoredProcedureReferencesSetId],[StoredProcsFolder],[TargetFramework],[TemplateVersion]
...
Go
-- =========================================================
-- Procure Name: DTNDatabase_FetchAllForProjectId
-- Author:           Data Juggler - Data Tier.Net Procedure Generator
-- Create Date:   9/17/2026
-- Description:    Returns all DTNDatabase objects for the ProjectId given.
-- =========================================================

-- Check if the procedure already exists
IF EXISTS (select * from syscomments where id = object_id ('DTNDatabase_FetchAllForProjectId'))

    -- Procedure Does Exist, Drop First
    BEGIN

        -- Execute Drop
        Drop Procedure DTNDatabase_FetchAllForProjectId

        -- Test if procedure was dropped
        IF OBJECT_ID('dbo.DTNDatabase_FetchAllForProjectId') IS NOT NULL

            -- Print Line Drop Failed
            PRINT '<<< Drop Failed On Procedure DTNDatabase_FetchAllForProjectId >>>'

        Else

            -- Print Line Procedure Dropped
            PRINT '<<< Drop Suceeded On Procedure DTNDatabase_FetchAllForProjectId >>>'

    End

GO

Create PROCEDURE DTNDatabase_FetchAllForProjectId
...
Go
-- =========================================================
-- Procure Name: DTNField_FetchAllForTableId
-- Author:           Data Juggler - Data Tier.Net Procedure Generator
-- Create Date:   9/17/2026
-- Description:    Returns all DTNField objects for the TableId given.
-- =========================================================

-- Check if the procedure already exists
IF EXISTS (select * from syscomments where id = object_id ('DTNField_FetchAllForTableId'))

    -- Procedure Does Exist, Drop First
    BEGIN

        -- Execute Drop
        Drop Procedure DTNField_FetchAllForTableId

        -- Test if procedure was dropped
        IF OBJECT_ID('dbo.DTNField_FetchAllForTableId') IS NOT NULL

            -- Print Line Drop Failed
            PRINT '<<< Drop Failed On Procedure DTNField_FetchAllForTableId >>>'

        Else

            -- Print Line Procedure Dropped
            PRINT '<<< Drop Suceeded On Procedure DTNField_FetchAllForTableId >>>'

    End

GO

Create PROCEDURE DTNField_FetchAllForTableId
...
Go
-- =========================================================
-- Procure Name: Enumeration_FetchAllForProjectId
-- Author:           Data Juggler - Data Tier.Net Procedure Generator
-- Create Date:   9/17/2026
-- Description:    Returns all Enumeration objects for the ProjectId given.
-- =========================================================

-- Check if the procedure already exists
IF EXISTS (select * from syscomments where id = object_id ('Enumeration_FetchAllForProjectId'))

    -- Procedure Does Exist, Drop First
    BEGIN

        -- Execute Drop
        Drop Procedure Enumeration_FetchAllForProjectId

        -- Test if procedure was dropped
        IF OBJECT_ID('dbo.Enumeration_FetchAllForProjectId') IS NOT NULL

            -- Print Line Drop Failed
            PRINT '<<< Drop Failed On Procedure Enumeration_FetchAllForProjectId >>>'

        Else

            -- Print Line Procedure Dropped
            PRINT '<<< Drop Suceeded On Procedure Enumeration_FetchAllForProjectId >>>'

    End

GO

Create PROCEDURE Enumeration_FetchAllForProjectId
...
Go
-- =========================================================
-- Procure Name: FieldSet_FetchAllForTableId
-- Author:           Data Juggler - Data Tier.Net Procedure Generator
-- Create Date:   9/17/2026
-- Description:    Returns all FieldSet objects for the TableId given.
-- =========================================================

-- Check if the procedure already exists
IF EXISTS (select * from syscomments where id = object_id ('FieldSet_FetchAllForTableId'))

    -- Procedure Does Exist, Drop First
    BEGIN

        -- Execute Drop
        Drop Procedure FieldSet_FetchAllForTableId

        -- Test if procedure was dropped
        IF OBJECT_ID('dbo.FieldSet_FetchAllForTableId') IS NOT NULL

            -- Print Line Drop Failed
            PRINT '<<< Drop Failed On Procedure FieldSet_FetchAllForTableId >>>'

        Else

            -- Print Line Procedure Dropped
            PRINT '<<< Drop Suceeded On Procedure FieldSet_FetchAllForTableId >>>'

    End

GO

Create PROCEDURE FieldSet_FetchAllForTableId
...
Go
-- =========================================================
-- Procure Name: FieldSetFieldView_FetchAllForFieldSetId
-- Author:           Data Juggler - Data Tier.Net Procedure Generator
-- Create Date:   9/17/2026
-- Description:    Returns all FieldSetFieldView objects for the FieldSetId given.
-- =========================================================

-- Check if the procedure already exists
IF EXISTS (select * from syscomments where id = object_id ('FieldSetFieldView_FetchAllForFieldSetId'))

    -- Procedure Does Exist, Drop First
    BEGIN

        -- Execute Drop
        Drop Procedure FieldSetFieldView_FetchAllForFieldSetId

        -- Test if procedure was dropped
        IF OBJECT_ID('dbo.FieldSetFieldView_FetchAllForFieldSetId') IS NOT NULL

            -- Print Line Drop Failed
            PRINT '<<< Drop Failed On Procedure FieldSetFieldView_FetchAllForFieldSetId >>>'

        Else

            -- Print Line Procedure Dropped
            PRINT '<<< Drop Suceeded On Procedure FieldSetFieldView_FetchAllForFieldSetId >>>'

    End

GO

Create PROCEDURE FieldSetFieldView_FetchAllForFieldSetId
...
Go
-- =========================================================
-- Procure Name: Method_FetchAllForProjectId
-- Author:           Data Juggler - Data Tier.Net Procedure Generator
-- Create Date:   9/17/2026
-- Description:    Returns all Method objects for the ProjectId given.
-- =========================================================

-- Check if the procedure already exists
IF EXISTS (select * from syscomments where id = object_id ('Method_FetchAllForProjectId'))

    -- Procedure Does Exist, Drop First
    BEGIN

        -- Execute Drop
        Drop Procedure Method_FetchAllForProjectId

        -- Test if procedure was dropped
        IF OBJECT_ID('dbo.Method_FetchAllForProjectId') IS NOT NULL

            -- Print Line Drop Failed
            PRINT '<<< Drop Failed On Procedure Method_FetchAllForProjectId >>>'

        Else

            -- Print Line Procedure Dropped
            PRINT '<<< Drop Suceeded On Procedure Method_FetchAllForProjectId >>>'

    End

GO

Create PROCEDURE Method_FetchAllForProjectId
...
Go
-- =========================================================
-- Procure Name: Method_FetchAllForTableId
-- Author:           Data Juggler - Data Tier.Net Procedure Generator
-- Create Date:   9/17/2026
-- Description:    Returns all Method objects for the TableId given.
-- =========================================================

-- Check if the procedure already exists
IF EXISTS (select * from syscomments where id = object_id ('Method_FetchAllForTableId'))

    -- Procedure Does Exist, Drop First
    BEGIN

        -- Execute Drop
        Drop Procedure Method_FetchAllForTableId

        -- Test if procedure was dropped
        IF OBJECT_ID('dbo.Method_FetchAllForTableId') IS NOT NULL

            -- Print Line Drop Failed
            PRINT '<<< Drop Failed On Procedure Method_FetchAllForTableId >>>'

        Else

            -- Print Line Procedure Dropped
            PRINT '<<< Drop Suceeded On Procedure Method_FetchAllForTableId >>>'

    End

GO

Create PROCEDURE Method_FetchAllForTableId
...
Go
-- =========================================================
-- Procure Name: Method_FindByName
-- Author:           Data Juggler - Data Tier.Net Procedure Generator
-- Create Date:   9/17/2026
-- Description:    Find an existing Method for the Name given.
-- =========================================================

-- Check if the procedure already exists
IF EXISTS (select * from syscomments where id = object_id ('Method_FindByName'))

    -- Procedure Does Exist, Drop First
    BEGIN

        -- Execute Drop
        Drop Procedure Method_FindByName

        -- Test if procedure was dropped
        IF OBJECT_ID('dbo.Method_FindByName') IS NOT NULL

            -- Print Line Drop Failed
            PRINT '<<< Drop Failed On Procedure Method_FindByName >>>'

        Else

            -- Print Line Procedure Dropped
            PRINT '<<< Drop Suceeded On Procedure Method_FindByName >>>'

    End

GO

Create PROCEDURE Method_FindByName
...
Go
-- =========================================================
-- Procure Name: CustomReader_FetchAllForTableId
-- Author:           Data Juggler - Data Tier.Net Procedure Generator
-- Create Date:   9/17/2026
-- Description:    Returns all CustomReader objects for the TableId given.
-- =========================================================

-- Check if the procedure already exists
IF EXISTS (select * from syscomments where id = object_id ('CustomReader_FetchAllForTableId'))

    -- Procedure Does Exist, Drop First
    BEGIN

        -- Execute Drop
        Drop Procedure CustomReader_FetchAllForTableId

        -- Test if procedure was dropped
        IF OBJECT_ID('dbo.CustomReader_FetchAllForTableId') IS NOT NULL

            -- Print Line Drop Failed
            PRINT '<<< Drop Failed On Procedure CustomReader_FetchAllForTableId >>>'

        Else

            -- Print Line Procedure Dropped
            PRINT '<<< Drop Suceeded On Procedure CustomReader_FetchAllForTableId >>>'

    End

GO

Create PROCEDURE CustomReader_FetchAllForTableId
