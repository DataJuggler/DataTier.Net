...
Go
-- =========================================================
-- Procure Name: Project_FindByProjectName
-- Author:           Data Juggler - Data Tier.Net Procedure Generator
-- Create Date:   1/28/2025
-- Description:    Find an existing Project for the ProjectName given.
-- =========================================================

-- Check if the procedure already exists
IF EXISTS (select * from syscomments where id = object_id ('Project_FindByProjectName'))

    -- Procedure Does Exist, Drop First
    BEGIN

        -- Execute Drop
        Drop Procedure Project_FindByProjectName

        -- Test if procedure was dropped
        IF OBJECT_ID('dbo.Project_FindByProjectName') IS NOT NULL

            -- Print Line Drop Failed
            PRINT '<<< Drop Failed On Procedure Project_FindByProjectName >>>'

        Else

            -- Print Line Procedure Dropped
            PRINT '<<< Drop Suceeded On Procedure Project_FindByProjectName >>>'

    End

GO

Create PROCEDURE Project_FindByProjectName

    -- Create @ProjectName Paramater
    @ProjectName nvarchar(50)
